...
void update(int ind, int k, int val, int low, int high, int seg[], vi &v)
{
    if (low == high && low == k)
    {
        v[low] = val;
    ...
    if (k >= low && k <= mid)
    {
        update(2 * ind + 1, k, val, low, mid, seg, v);
    }
    else
    {
        update(2 * ind + 2, k, val, mid + 1, high, seg, v);
    }
    seg[ind] = seg[2 * ind + 1] + seg[2 * ind + 2];
}
int query(int ind, int l, int r, int left, int right, int seg[])
{
    if (left > r || right < l)
    {
        return 0;
    ...
    int ans1 = query(2 * ind + 1, l, r, left, mid, seg);
    int ans2 = query(2 * ind + 2, l, r, mid + 1, right, seg);
    return (ans1 + ans2);
}
void build(int ind, int l, int r, int seg[], vi &v)
{
    if (l == r)
    {
        seg[ind] = v[l];
    ...
    int mid = l + (r - l) / 2;
    build(2 * ind + 1, l, mid, seg, v);
    build(2 * ind + 2, mid + 1, r, seg, v);
    seg[ind] = seg[2 * ind + 1] + seg[2 * ind + 2];
}
#define read(v, n)              \
    for (int i = 0; i < n; i++) \
