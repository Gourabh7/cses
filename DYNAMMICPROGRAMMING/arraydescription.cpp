...
int f(int ind, int prev, vi &v, vector<vector<int>> &dp, int m, int n)
{
    if (ind == n)
    {
        return 1;
    ...
    if (prev == -1 && v[ind] == 0)
    {
        for (int i = 1; i <= m; i++)
        {
            ans = (ans % mod + f(ind + 1, i, v, dp, m, n) % mod) % mod;
        }
    }
    else if (prev == -1 && v[ind] != 0)
    {
        ans = (ans % mod + f(ind + 1, v[ind], v, dp, m, n) % mod) % mod;
    }
    else if (v[ind] == 0)
    {
        if ((prev + 1) <= m && (prev + 1) >= 1)
            ans = (ans % mod + f(ind + 1, prev + 1, v, dp, m, n) % mod) % mod;
        ...
        if (prev >= 1 && prev <= m)
        {
            ans = (ans % mod + f(ind + 1, prev, v, dp, m, n) % mod) % mod;
        }
    }
    else
    {
        if (abs(prev - v[ind]) <= 1 && prev <= m && prev >= 1)
        {
            ans = (ans % mod + f(ind + 1, v[ind], v, dp, m, n) % mod) % mod;
        }
        else
