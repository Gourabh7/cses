...
int f(int i,int j,int n,vector<vector<char>>&mat,vector<vi>&dp){
...
return 1;
    ...
    int left =0;
    if(j<(n-1) && mat[i][j+1]=='.'){
        left  = f(i,j+1,n,mat,dp)%mod;
    }
    int right = 0;
    if(i<(n-1) && mat[i+1][j]=='.'){
        right = f(i+1,j,n,mat,dp)%mod;
    }
    return dp[i][j] =  (left%mod+right%mod)%mod;
