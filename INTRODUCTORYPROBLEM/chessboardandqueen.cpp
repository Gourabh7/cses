...
int f(int ind,vector<string>&v,vi &col,vi &lowerdiag,vi &upperdiag){
        if(ind==8){
            return 1;
        }
        int ans = 0;
        for(int j = 0;j<8;j++){
                if(v[ind][j]=='*' || v[ind][j]=='q' || col[j] || lowerdiag[(ind+j)] || upperdiag[7+ind-j] ){
                    continue;
                }
                    v[ind][j] = 'q';
                    col[j] = 1;
                    lowerdiag[ind+j] = 1;
                    upperdiag[7+ind-j] = 1;
                    ans += f(ind+1,v,col,lowerdiag,upperdiag);
                    v[ind][j] = '.';
                    col[j] = 0;
                    lowerdiag[ind+j] = 0;
                    upperdiag[7+ind-j] = 0;
...
void solve()
{
    ...
    vi upperdiag(20,0);
    vi lowerdiag(20,0);
    int ans = f(0,v,col,lowerdiag,upperdiag);
    cout<<ans<<endl;
}
main()  {
fast_io;
