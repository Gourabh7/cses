...
void dfs(int node,vector<vector<int>>&adj,vi &vis,vi &ans,vi &sub){
    vis[node]=1;
    int cnt=0;
    int val=1;
    for(auto it:adj[node]){
        if(!vis[it]){
            dfs(it,adj,vis,ans,sub);
            cnt+=sub[it]+ans[it];
            val+=sub[it];
        }
    }
...
void solve(vi &ans,vi  &sub,vector<vector<int>>&adj,int src,int par,int &totalnodes,int parans){
ans[src]=ans[src]+(parans+totalnodes-sub[src]);
for(auto it:adj[src]){
    if(par!=it){
        solve(ans,sub,adj,it,src,totalnodes,ans[src]-(sub[it]+ans[it]));
    }
}
}
...
vi sub(n+1,0);
vi vis(n+1,0);
dfs(1,adj,vis,ans,sub);
// out(sub);
// out(ans);
solve(ans,sub,adj,1,0,n,0);
for(int i =1;i<=n;i++){
    cout<<ans[i]<<" ";
}
cout<<nl;
